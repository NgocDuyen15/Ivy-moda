...
message.leftSlider-=165,message.leftSlider-165<-length&&(message.leftSlider=-length+165)},$scope.doInitDomain=function(domain,parentUsername){DomainService.getChat(domain,parentUsername).then(function(data){if(null!=data&&void 0!=data&&(!data.data||1!=data.data.is_banned)){$scope.useFacebook=1==data.data.useFacebook,null!=data.data.pageId&&""!=data.data.pageId&&($scope.pageId=data.data.pageId),$scope.useFacebook||(CS_WEB_PUSH=data.data.chatServer,uploadUrl=data.data.uploadUrl,webPush.connect(CS_WEB_PUSH,0)),$scope.lstAdditionField=data.data.lstAdditionField
